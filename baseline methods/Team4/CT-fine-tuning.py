...
NN fine tuning script

Modified version of CT-predict-pretrain.ipynb
...
# Returns model based on its name (model must be in model_names)
def getModel(modelName):
    if modelName == "Dense169_self_trans":
        model = model_names[modelName]

    else: model = torch.load(model_names[modelName])
